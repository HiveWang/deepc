...
class Quadcopter:
    ...
    def dynamics(self, state, t, u):
        ...
        return np.concatenate([v, acceleration, rpy_dot, omega_dot])
...
class DeePC:
    ...
    def __init__(self, Td=200, Tini=4, Tf=15, 
                 lambda_g=50, lambda_s=1e6, p=3,
                 epsilon=0.001, lambda_ini=1e5):
        # 优化后的参数
        self.Td = Td      
        self.Tini = Tini  
        self.Tf = Tf      
        ...
        self.epsilon = epsilon  # Wasserstein球半径
        self.lambda_ini = lambda_ini  # 初始条件正则化系数
        self.p = p        
        self.H = np.empty((0, 0))
        ...
        # 约束参数
        self.u_min = np.array([0.22, -0.25, -0.25])   # 最小推力0.25N (防止下坠)
        self.u_max = np.array([0.38, 0.25, 0.25])    # 最大推力0.35N (防止上升过快)
        self.y_min = np.array([-1.5, -1.5, 0.8]) 
        self.y_max = np.array([1.5, 1.5, 2.0])

        # 成本矩阵权重
        self.Q = np.diag([30, 30, 30])
        self.R = np.diag([10, 1, 1])
        
    def wasserstein_robust_term(self, g, h):
        """计算Wasserstein鲁棒项（文献定理4.2）"""
        # 简化实现 - 实际应根据系统特性调整
        term1 = self.lambda_ini * cp.norm(h, 1)
        term2 = self.epsilon * cp.norm(g, 1)
        return cp.maximum(term1, term2)
    ...
    def build_hankel(self, ud, yd):
        ...
        # 持久激励检查（文献III-A节）
        min_length = (self.m + 1) * L - 1
        if self.Td < min_length:
            raise ValueError(f"数据不足! 需要Td≥{min_length}, 当前Td={self.Td}")
        ...
        H = np.vstack([Hu, Hy])
        return H, np.ones((H.shape[1], 1))  # 简化标准化
    ...
    def solve(self, H, uini, yini, ref_traj, current_state):
        ...
        sigma_y = cp.Variable(self.Tini * n_y)  # 松弛变量
        u = cp.Variable((self.Tf, n_u))
        y = cp.Variable((self.Tf, n_y))
        h = cp.Variable(self.Tini * n_y)  # Wasserstein相关变量
        
        # 3. 约束条件（文献公式7）
        constraints = [
            Up @ g == uini.flatten(),
            Yp @ g == yini.flatten() + sigma_y,  # 松弛输出约束
            Uf @ g == u.flatten(),
            Yf @ g == y.flatten(),
        ...
        for k in range(self.Tf):
            # 跟踪误差（L1范数 - 文献V节）
            base_cost += 200 * cp.norm(y[k, :] - ref_traj[k], 1)
            # 控制输入（L1范数）
            base_cost += cp.norm(u[k], 1)
        
        # 正则化项（文献公式8）
        reg_cost = self.lambda_g * cp.norm(g, 1) + self.lambda_s * cp.norm(sigma_y, 1)
        
        # Wasserstein鲁棒项（文献定理4.2）
        robust_term = self.wasserstein_robust_term(g, h)
        
        # 完整目标函数
        cost = base_cost + reg_cost + robust_term
...
def deepc_simulation():
    ...
    param_sets = {
        ...
        8: {'Td': 214, 'Tini': 2, 'Tf': 30, 'lambda_g': 50, 'lambda_s': 1e6, 'p': 3,
            'Q_diag': [25, 25, 25], 'R_diag': [30, 3, 3]}
